...
//Filter Coefficients: [-0.0051643  -0.02288252  0.09675565  0.43129117  0.43129117  0.09675565
// -0.02288252 -0.0051643 ]
bit tx_complete = 0;
bit rx_complete = 0;
int sample_arr[]={0,0,0,0,0,0,0,0};
double filter_arr[]= {0.00875474, 0.04794887, 0.16402439, 0.27927199, 0.27927199, 0.16402439,
...
void main(void)
{
	...
	while (1)
	{
		...
		while(count>=0){
			sample_arr[count+1] = sample_arr[count];
			count--;
		}
		count=0;
		sample_arr[0]=sample_val;
		
		sample_val_doub = 0;
		while(count<8){
			sample_val_doub=sample_val_doub + sample_arr[count] * filter_arr[count];
			count++;
		}
		count=6;
		sample_val_doub = (int)sample_val_doub;
