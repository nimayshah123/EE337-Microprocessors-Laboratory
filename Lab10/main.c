...
bit tx_complete = 0;
bit rx_complete = 0;
float coeff[8]={0.02058851,0.06541966,0.16640878,0.24758305,0.24758305,0.16640878,0.06541966,0.02058851}
int 
...
void main(void)
{
	...
	while (1)
	{
		...
		sample_sum += sample_val*coeff[i];
	}
		//converting integer value back to 4 character array (i.e string)
		data_send[0] = 48 + (sample_sum/1000);
		data_send[1] = 48 + (sample_sum%1000/100);
		data_send[2] = 48 + (sample_sum%100/10);
		data_send[3] = 48 + (sample_sum%10);
